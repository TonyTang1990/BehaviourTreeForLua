...
namespace Tutorial
{
    public class CSCallLua : MonoBehaviour
    {
        ...
        LTable = {}
        LTable.cstestclass = nil
        function LTable:SetCSTestClass(testclass)
            print('LTable:SetCSTestClass()')
            print(testclass)
            self.cstestclass = testclass
            self.cstestclass.LB = self
            self.cstestclass.MemberDelegate1 = self.LuaMemberFunc1
            self.cstestclass.MemberDelegate2 = self.LuaMemberFunc2
        end

        function LTable:LuaMemberFunc1()
            print('LTable:LuaMemberFunc1()')
        end

        function LTable:LuaMemberFunc2(value)
            print('LTable:LuaMemberFunc2()')
            print(value)
        end            
        ";
        ...
        {
            public Action<LuaTable> MemberDelegate1;
            public Action<LuaTable, int> MemberDelegate2;

            public LuaTable LB;

            public void MemberFunc1()
            {
                Debug.Log("MemberFunc1()");
                MemberDelegate1.Invoke(LB);
            }

            public void MemberFunc2(int value)
            {
                Debug.Log($"MemberFunc2({value})");
                MemberDelegate2.Invoke(LB, value);
            }
        }
        ...
        void Start()
        {
            ...
            Debug.Log("==================================");
            var cstestclassinstance = new TestClass();
            var ltable = luaenv.Global.Get<LuaTable>("LTable");
            var setcstestclass = ltable.Get<Action<LuaTable, TestClass>>("SetCSTestClass");
            setcstestclass(ltable, cstestclassinstance);
            cstestclassinstance.MemberFunc1();
            cstestclassinstance.MemberFunc2(2);
            Debug.Log("==================================");
